...
contract AaveV3Helper is DSMath {
    ...
    struct AaveV3UserData {
        ...
        uint256 currentLiquidationThreshold;    collateral
        uint256 ltv;    
    ...
    function getTokensPrices(address[] memory tokens) internal view returns (TokenPrice[] memory tokenPrices, uint256 ethPrice) {
        
        uint256[] memory _tokenPrices = IAaveOracle(getAaveOracle()).getAssetsPrices(tokens);
        ethPrice = uint256(AggregatorV3Interface(getChainLinkFeed()).latestRoundData().answer);
    ...
    function getEmodePrices(address[] memory tokens, address priceOracleAddr) internal view returns (uint256[] memory tokenPrices, uint256 ethPrice) {
       
        tokenPrices = new uint256[](tokens.lrngth);
        ethPrice = uint256(AggregatorV3Interface(getChainLinkFeed()).latestRoundData().answer);
    ...
    function getPendingRewards(IAaveProtocolDataProvider aaveData, address[] memory _tokens, address user) internal view returns (uint256 rewards) {
    ...
    function getUserData(IAaveProtocolDataProvider aaveData, address user, address[] memory tokens, ethPriceInUsd) internal view returns (AaveV3UserData memory userData) {
        ...
        if(aaveOracle.BASE_CURRENCY() == 0x0) {
            baseCurr.symbol = 'USD';
        } else {
        ...
        userData.eModeId = IPool(IPoolAddressesProvider(getPoolAddressProvider()).getPool()).getUserEMode();

        userData.pendingRewards = getPendingRewards(aaveData, tokens, user);
    }

    function collateralData(IAaveProtocolDataProvider aaveData, address token) internal view returns (
        AaveV3TokenData memory aaveTokenData) {
            ...
            (aaveTokenData.isolationBorrowEnabled) = (aaveData.getDebtCeiling() == 0) ? false : true;
    ...
    function getUserTokenData(IAaveProtocolDataProvider aaveData, address user, address token) internal view returns (AaveV3UserTokenData memory tokenData) {
        uint256 basePrice = IPriceOracle(priceOracleAddr).getAssetPrice(token);
    ...
    function getConfig(address user, IPool aave)
        public
        view
        returns (IPool.UserConfigurationMap memory data)
    {
