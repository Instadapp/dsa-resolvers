...
contract MorphoResolver is MorphoHelpers {
     ...
     *@param pool Underlying protocol, Aave or Compound.
     *@return positionData_ Overall position details of the user - balances, rewards, collaterals, market details.
     */
    function getPositionAll(Underlying pool, address user) public view returns (UserData memory positionData_) {
        address[] memory userMarkets_ = getUserMarkets(user, pool);
        positionData_ = getUserData(user, userMarkets_, pool);
    }
    ...
    function getPosition(
        Underlying pool,
        address user,
        address[] memory userMarkets
    ) public view returns (UserData memory positionData_) {
        positionData_ = getUserData(user, userMarkets, pool);
    }
...
contract InstaMorphoResolver is MorphoResolver {
    string public constant name = "Morpho-Resolver-v1.0";
}
