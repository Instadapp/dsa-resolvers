...
abstract contract Helpers is DSMath {
     ...
     */
    address internal constant wavaxAddr = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
    ...
    function _getTokenBalance(address token, address owner) internal view returns (uint256) {
        uint256 balance = token == wavaxAddr ? owner.balance : TokenInterface(token).balanceOf(owner);
        return balance;
    }
    ...
    function _getPoolData(address lpTokenAddr, address owner) internal view returns (PoolData memory pool) {
        ...
        pool = PoolData(
            tokenA == getAddressWAVAX() ? getAvaxAddr() : tokenA,
            tokenB == getAddressWAVAX() ? getAvaxAddr() : tokenB,
            address(lpToken),
            reserveA,
            reserveB,
            amtA,
