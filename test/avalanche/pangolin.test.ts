...
describe("Pangolin Exchange", async () => {
  ...
  describe("Pangolin Exchange Resolver", async () => {
    ...
    it("Returns buy amount", async () => {
      const [buyAmt, unitAmt] = await pangolin.getBuyAmount(
    ...
    it("Returns sell amount", async () => {
      const [sellAmt, unitAmt] = await pangolin.getSellAmount(
        "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
        "0x60781C2586D68229fde47564546784ab3fACA982",
        ethers.utils.parseEther("10"),
        BigNumber.from("3"),
    ...
    it("Returns deposit amount", async () => {
      const [amountB, uniAmount, amountAMin, amountBMin] = await pangolin.getDepositAmount(
        "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
        "0x60781C2586D68229fde47564546784ab3fACA982",
        ethers.utils.parseEther("10"),
        BigNumber.from("3"),
    ...
    it("Returns single deposit amount", async () => {
      const [amtA, amtB, uniAmt, minUniAmt] = await pangolin.getSingleDepositAmount(
        "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
        "0x60781C2586D68229fde47564546784ab3fACA982",
        ethers.utils.parseEther("10"),
        BigNumber.from("3"),
    ...
    it("Returns withdraw amounts", async () => {
      const [amtA, amtB, unitAmtA, unitAmtB] = await pangolin.getWithdrawAmounts(
        "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
        "0x60781C2586D68229fde47564546784ab3fACA982",
        ethers.utils.parseEther("10"),
        BigNumber.from("3"),
    ...
    it("Returns the getPositionByPair", async () => {
      const [position1] = await pangolin.getPositionByPair(signer.address, [
    ...
    it("Returns the getpooldata", async () => {
      const [
        ...
        lpAmount,
        totalSupply,
      ] = await pangolin.getpooldata("0xd7538cABBf8605BdE1f4901B47B8D42c61DE0367", signer.address);
    ...
    it("Returns the getPosition", async () => {
      const [position1] = await pangolin.getPosition(signer.address, ["0xd7538cABBf8605BdE1f4901B47B8D42c61DE0367"]);
      const [
        tokenA,
        tokenB,
        lpAddress,
