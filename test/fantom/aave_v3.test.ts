    ...
    it("Returns the user's positions on AaveV3 for all assets", async () => {
      ...
      for (let i = 0; i < tokenData.length; i++) {
        ...
        for (let j = 0; j < rewards[i].sIncentiveData.userRewards.rewardsToken.length; j++) {
          console.log(
            `Unclaimed Reward: ${rewards[i].sIncentiveData.userRewards.unbalancedAmounts[j]} for ${rewards[i].aIncentiveData.userRewards.rewardsToken[j]}`,
          );
        }
        console.log("----------------------------------------------------");
        ...
        for (let j = 0; j < rewards[i].vIncentiveData.userRewards.rewardsToken.length; j++) {
          console.log(
            `Unclaimed Reward: ${rewards[i].vIncentiveData.userRewards.unbalancedAmounts[j]} for ${rewards[i].aIncentiveData.userRewards.rewardsToken[j]}`,
          );
        }
        console.log();
