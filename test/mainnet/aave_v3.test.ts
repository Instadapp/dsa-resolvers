...
import { InstaAaveV3Resolver, InstaAaveV3Resolver__factory } from "../../typechain";

describe("AaveV3", () => {
  let signer: SignerWithAddress;
  const account = "0x9600A48ed0f931d0c422D574e3275a90D8b22745";
  const AAVE_V3_POOL_ADDRESS_PROVIDER = "0x2f39d218133AFaB8F2B819B1066c7E434Ad94E9e";

  before(async () => {
    [signer] = await ethers.getSigners();
  });

  describe("AaveV3 Resolver", () => {
    let resolver: InstaAaveV3Resolver;
    before(async () => {
      ...
      const deployer = new InstaAaveV3Resolver__factory(signer);
      resolver = await deployer.deploy();
      await resolver.deployed();
    });

    it("should get user configurations and reserves list", async () => {
      const reservesList = await resolver.getReservesList(AAVE_V3_POOL_ADDRESS_PROVIDER);
      const reserves = await resolver.getConfiguration(account, AAVE_V3_POOL_ADDRESS_PROVIDER);
    ...
    it("Returns the positions on Aave V3", async () => {
      ...
      console.log("Supply Balance wstETH: ", formatUnits(userTokenData[4].supplyBalance, "18"));
      //   expect(userTokenData[0].supplyBalance).to.gte(0);
      console.log("Variable Borrow Balance ETH: ", formatUnits(userTokenData[3].variableBorrowBalance, "18"));
    ...
    it("Returns List of Tokens", async ()=> {
        const list = await resolver.callStatic.getList(AAVE_V3_POOL_ADDRESS_PROVIDER);
        console.log("List of Tokens: ", list);
    })

    it("Returns the user's positions on Aave V3 for all assets", async () => {
      const results = await resolver.callStatic.getPositionAll(account, AAVE_V3_POOL_ADDRESS_PROVIDER);
      const userTokenData = results[1];
      const tokenData = results[2];
      const userData = results[0];
    ...
    it("Returns the e-mode category details of e-modeID", async () => {
      const emodeData = await resolver.getEmodeCategoryData(1, AAVE_V3_POOL_ADDRESS_PROVIDER);
      console.log();
      console.log("*************************************************");
